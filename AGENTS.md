...
Manifest V3 badge-only extension. Displays the current Bitcoin fee (sat/vB) for ~6-block (1-hour) confirmation on the toolbar badge.
...
- **Badge-only display** — `updateBadge(feeText)` truncates to 4 chars max for badge readability.
...
- **Cache with TTL** — `fetchBitcoinFee(forceUpdate=false)` checks `chrome.storage.local` for a cached value with 30-minute TTL before hitting the API.
- **API:** `GET https://blockstream.info/api/fee-estimates` — extracts `data['6']` (6-block/1h confirmation fee in sat/vB).
...
- **Alarm** — `chrome.alarms` fires `'updateBitcoinFee'` every 30 minutes.
- **Storage keys in `chrome.storage.local`:** `bitcoinFee` (number), `bitcoinFeeTimestamp` (epoch ms).
